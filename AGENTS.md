...
30 days. Entries older than 30 days are excluded from `getCachedScan` queries. No background cleanup — stale entries remain in D1 until manually pruned.
...
| `X-Cache-Access-Count` | `<integer>` | How many times this cache entry has been served (HIT only) |
